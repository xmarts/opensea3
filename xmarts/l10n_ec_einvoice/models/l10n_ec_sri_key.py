...
class L10nEcSriKey(models.Model):
	_name = "l10n.ec.sri.key"
	_description = "Electronic Key"
	...
	file_content = fields.Binary(
		string="Electronic Sign File",
		readonly=True,
		states=STATES
	)
	file_name = fields.Char(
		string="Electronic Sign Filename",
		readonly=True
	)
	password = fields.Char(
		string="Electronic Sign Password",
		readonly=True,
		states=STATES
	)
	...
	subject_serial_number = fields.Char(
		string="Numero de Serie(Asunto)",
		readonly=True
	)
	subject_common_name = fields.Char(
		string="Organizacion(Asunto)",
		readonly=True
	)
	issuer_common_name = fields.Char(
		string="Organizacion(Emisor)",
		readonly=True
	)
	cert_serial_number = fields.Char(
		string="Numero de serie(cerificado)",
		readonly=True
	)
	cert_version = fields.Char(
		string="Version",
		readonly=True
	)
...
#			"expire_date": datetime.strptime(cert.get_notAfter().decode("utf-8"), "%Y%m%d%H%M%SZ"),
	...
	def convert_key_cer_to_pem(self, key, password):
		...
		with tempfile.NamedTemporaryFile(
			...
			subprocess.call(
				(KEY_TO_PEM_CMD %
				 (key_file.name, keypem_file.name, password, password)).split())
	...
	def action_sign(self, xml_string_data):
		...
		try:
			...
			raise UserError(
				_(
					"Error opening the signature, possibly the signature"
					" key has been entered incorrectly or the file "
					"is not supported. \n%s "
				) % (tools.ustr(ex))
			)
